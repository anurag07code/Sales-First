...
interface AIAssistantPanelProps {
  ...
  showCompare?: boolean;
  variant?: "default" | "detail"; // Different appearance for different pages
}
...
const AIAssistantPanel = ({
    ...
    setMessages([
      {
        ...
        content: selectedProject
          ? `I've switched to "${selectedProject.rfpTitle}". What would you like to know about this RFP?`
          : "Please select an RFP document to continue.",
      },
    ]);
  };
  ...
  return (
    <>
      ...
      <Button
        size="icon"
        className={`fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-elegant z-[950] transition-all ${
          variant === "detail" 
      ...
      {isOpen && (
        <>
          ...
          <div
            className="fixed inset-0 bg-black/50 z-[900] block md:hidden"
            onClick={() => setIsOpen(false)}
          />

          {/* Panel */}
          <div
            className="fixed right-4 md:right-[88px] top-6 md:top-24 bottom-4 md:bottom-6 w-[calc(100%-2rem)] md:w-96 rounded-2xl border border-border z-[999] flex flex-col shadow-2xl overflow-hidden"
            style={{ backgroundColor: 'hsl(var(--card))' }}
          >
            {/* Header */}
            <div 
              className={`p-4 border-b border-border sticky top-0 z-10 ${
                variant === "detail" ? "border-green-200" : "border-border"
              }`}
              style={{ backgroundColor: 'hsl(var(--card))' }}
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <button
                  aria-label="Close"
                  className="ml-auto h-8 w-8 rounded-full flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/40"
                  onClick={() => setIsOpen(false)}
                >
                  <X className="h-4 w-4" />
                </button>
                ...
                <div className="space-y-2 mt-3">
                  {!compareOpen ? (
                    <Button
                      size="sm"
                      variant="outline"
                      className="w-full border-green-300 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20"
                    ...
                    <>
                      ...
                      <Select 
                        ...
                        onValueChange={(value) => setCompareProjectId(value === "none" ? "" : value)}
                      >
                        <SelectTrigger className="w-full bg-background">
                          <SelectValue placeholder="Select RFP to compare">
                            {compareProjectId
                              ? projects.find((p) => p.id === compareProjectId)?.rfpTitle
