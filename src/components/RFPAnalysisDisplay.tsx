...
const RFPAnalysisDisplay = ({ data, fileName }: RFPAnalysisDisplayProps) => {
  // Local editable copy
  const [localData, setLocalData] = useState<RFPEstimation>(data);
  useEffect(() => {
    setLocalData(data);
  }, [data]);
  ...
  const openSectionEditor = (
    ...
    setSectionDraft((localData as any)[section] || "");
  };

  const saveSectionEditor = () => {
    if (!editingSection) return;
    setLocalData((prev) => ({ ...prev, [editingSection]: sectionDraft }));
  ...
  const formatTextContent = (text: string) => {
    ...
    let currentSection: { type: 'heading' | 'paragraph' | 'list'; content: string } | null = null;
    ...
    lines.forEach(line => {
      ...
      if (trimmed.match(/^[A-Z][^:]*:$/)) {
        ...
        currentSection = { type: 'heading', content: trimmed.replace(':', '') };
      }
      // Detect numbered lists
      else if (trimmed.match(/^\d+[\.\)]\s/)) {
        if (currentSection && currentSection.type !== 'list') {
          sections.push(currentSection);
          currentSection = { type: 'list', content: trimmed };
    ...
    <div className="space-y-6">
      ...
      <Accordion type="multiple" className="space-y-3">
            ...
            <Card className="gradient-card overflow-hidden border-2 border-primary/20 hover:border-primary/40 transition-colors">
              ...
              <AccordionContent className="px-6 pb-6">
                {editingSection === "purpose" && (
                  <div className="mb-4 p-3 rounded-md border border-primary/20 bg-card">
                    <Textarea
                      value={sectionDraft}
                ...
                <div className="space-y-4 pt-2">
                  {(() => {
                    ...
                    return filteredSections.map((section, idx) => {
                      if (section.type === 'heading') {
                        return (
                          <div key={idx} className="flex items-center gap-2 mb-3 mt-4 first:mt-0">
                      ...
                      if (section.type === 'list') {
                        ...
                        return (
                          <div key={idx} className="space-y-2.5 pl-2">
                      ...
                      if (idx === firstParagraphIdx) {
                        return (
                          <div key={idx} className="p-4 rounded-lg bg-gradient-to-br from-muted/50 to-muted/30 border-2 border-border shadow-md">
                      ...
                      return (
                        <div key={idx} className="p-4 rounded-lg bg-gradient-to-r from-primary/5 via-primary/3 to-transparent border-l-4 border-primary/30">
        ...
        {localData.estimation?.rolesAndEfforts && (
          <AccordionItem value="estimation" className="border-none">
            <Card className="gradient-card overflow-hidden border-2 border-primary/20 hover:border-primary/40 transition-colors">
              <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-gradient-to-r hover:from-primary/5 hover:to-transparent transition-all">
                <div className="flex items-center gap-3 flex-1">
                  <div className="p-2.5 rounded-xl bg-gradient-to-br from-primary/20 to-primary/10 shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 pt-2">
                  {Object.entries(localData.estimation.rolesAndEfforts)
                    ...
                    .map(([role, hours], idx) => (
                      <div
                        key={role}
                        className="flex flex-col p-3 rounded-lg bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 hover:border-primary/40 hover:shadow-lg transition-all group cursor-pointer"
                      >
        ...
        {localData.technicalArchitecture && (
          <AccordionItem value="tech-arch" className="border-none">
            <Card className="gradient-card overflow-hidden border-2 border-primary/20 hover:border-primary/40 transition-colors">
              <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-gradient-to-r hover:from-primary/5 hover:to-transparent transition-all">
                <div className="flex items-center gap-3 flex-1">
                  <div className="p-2.5 rounded-xl bg-gradient-to-br from-primary/20 to-primary/10 shadow-sm">
              ...
              <AccordionContent className="px-6 pb-6">
                <div className="space-y-5 pt-2">
                  {localData.technicalArchitecture.technicalArchitectureOverview && (
                    <div className="p-5 rounded-xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border-2 border-primary/20 shadow-sm">
                  ...
                  {localData.technicalArchitecture.keyComponentsoftheTechnicalArchitecture && (
                    <div className="p-5 rounded-xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border-2 border-primary/20 shadow-sm">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                        {Object.entries(
                          ...
                          <div key={key} className="p-3 rounded-lg bg-card border-l-4 border-primary/40 hover:border-primary/60 transition-colors group">
                  ...
                  {localData.technicalArchitecture.implementationPlan && (
                    <div className="p-5 rounded-xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border-2 border-primary/20 shadow-sm">
                      <div className="flex items-center gap-2 mb-4">
                      ...
                      <div className="relative mb-6">
                        ...
                        <div className="space-y-6 relative">
                          {Object.entries(localData.technicalArchitecture.implementationPlan).map(
                            ([phase, description], idx) => {
                              return (
                  ...
                  {localData.technicalArchitecture.conclusion && (
                    <div className="p-5 rounded-xl bg-gradient-to-br from-primary/20 to-primary/10 border-2 border-primary/30 shadow-md">
                      <div className="flex items-center gap-2 mb-2">
        ...
        {localData.businessArchitecture && (
          <AccordionItem value="business-arch" className="border-none">
            <Card className="gradient-card overflow-hidden border-2 border-primary/20 hover:border-primary/40 transition-colors">
              <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-gradient-to-r hover:from-primary/5 hover:to-transparent transition-all">
                <div className="flex items-center gap-3 flex-1">
                  <div className="p-2.5 rounded-xl bg-gradient-to-br from-primary/20 to-primary/10 shadow-sm">
              ...
              <AccordionContent className="px-6 pb-6">
                <div className="space-y-5 pt-2">
                  ...
                  {localData.businessArchitecture.implementationPlan && (
                    <div className="p-5 rounded-xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border-2 border-primary/20 shadow-sm">
                      <div className="flex items-center gap-2 mb-4">
                        <Clock className="h-4 w-4 text-primary" />
                        <h4 className="font-bold text-base text-primary">Implementation Plan</h4>
                      ...
                      <div className="relative">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative">
                          {Object.entries(localData.businessArchitecture.implementationPlan).map(
                            ([phase, description], idx) => {
                              return (
              ...
              <AccordionContent className="px-6 pb-6">
                <div className="space-y-5 pt-2">
                  {localData.solutionTimeline.timelineOverview && (
                    <div className="p-4 rounded-xl bg-gradient-to-r from-primary/5 via-primary/3 to-transparent border-l-4 border-primary/30">
                      <div className="flex items-center gap-2 mb-2">
                        <Sparkles className="h-4 w-4 text-primary" />
                  ...
                  {localData.solutionTimeline.timelines.map((timeline, index) => (
                    <div key={index} className="space-y-3">
                      {Object.entries(timeline).map(([phase, details]) => (
                        <div key={phase} className="p-5 rounded-xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border-2 border-primary/20 shadow-sm">
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            {Object.entries(details).map(([key, value]) => (
                              <InlineTimelineEditor
                                ...
                                onSave={(v) => {
                                  ...
                                  const objIdx = timelines[index] as Record<string, any>;
                                  const entries = Object.entries(objIdx[phase] || {});
                                  const updated: Record<string, any> = {};
                                  entries.forEach(([k, val]) => {
                                    updated[k] = k === key ? v : val;
                                  });
                                  const updatedPhaseObj = { ...objIdx, [phase]: updated };
                                  timelines[index] = updatedPhaseObj as any;
                                  setLocalData((prev) => ({
                                    ...prev,
                                    solutionTimeline: { ...prev.solutionTimeline!, timelines },
                                  }));
                                }}
                              />
                            ))}
        ...
        {localData.keyRequirements && (
          <AccordionItem value="requirements" className="border-none">
            <Card className="gradient-card overflow-hidden border-2 border-primary/20 hover:border-primary/40 transition-colors">
              ...
              <AccordionContent className="px-6 pb-6">
                {editingSection === "keyRequirements" && (
                  <div className="mb-4 p-3 rounded-md border border-primary/20 bg-card">
                    <Textarea
                      value={sectionDraft}
                      onChange={(e) => setSectionDraft(e.target.value)}
                      ...
                      return (
                        <div key={idx} className="space-y-2.5 pl-2">
                          {items.map((item, itemIdx) => {
                            const cleanItem = item.replace(/^[\d•\-\*\)\.]\s*/, '').trim();
                            return (
                              <div key={itemIdx} className="flex items-start gap-3 group p-2 rounded-lg hover:bg-primary/5 transition-colors">
                                <div className="mt-1.5 p-1 rounded-full bg-primary/10 group-hover:bg-primary/20 transition-colors">
                                  <CheckCircle2 className="h-3 w-3 text-primary flex-shrink-0" />
                                </div>
                                <p className="text-sm text-foreground leading-relaxed flex-1 pt-0.5">
...
const InlineHoursEditor = ({ value, onChange }: InlineHoursEditorProps) => {
  ...
  if (!editing) {
    return (
      <Button
        variant="secondary"
        size="sm"
        className="h-6 px-2 gap-1 bg-primary text-primary-foreground border-primary/30"
        onClick={(e) => {
          e.stopPropagation();
          setEditing(true);
        }}
...
const InlinePlanEditor = ({ text, onSave }: InlinePlanEditorProps) => {
  const [editing, setEditing] = React.useState(false);
  const [draft, setDraft] = React.useState<string>(text);
  ...
  return (
    <div className="p-2 rounded-md border border-primary/20 bg-card" onClick={(e) => e.stopPropagation()}>
...
const InlineTimelineEditor = ({ label, text, onSave }: InlineTimelineEditorProps) => {
  const [editing, setEditing] = React.useState(false);
  const [draft, setDraft] = React.useState<string>(text);
  React.useEffect(() => setDraft(text), [text]);

  return (
    <div className="flex items-start gap-3 p-2.5 rounded-lg bg-card border border-primary/10 hover:border-primary/30 transition-colors group">
