...
import { Building2, FileText, FileCheck, TrendingUp, ArrowRight, Star, Zap, Sparkles, Shield, Users, Award, Target, BarChart3, Clock, Globe, Brain, Rocket, ArrowUpRight, Sparkle } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-8">
              {/* Badge */}
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border-2 transition-all duration-300 hover:border-primary hover:shadow-sm" style={{
              borderColor: '#3B82F6',
              backgroundColor: 'rgba(59, 130, 246, 0.05)'
            }}>
                <Sparkle className="w-4 h-4 transition-transform duration-300 hover:scale-110" style={{
                color: '#3B82F6'
              }} />
                <span className="text-sm font-medium transition-colors duration-300 hover:text-primary" style={{
                color: '#3B82F6'
              }}>AI-Powered Sales Intelligence Platform</span>
              </div>
              ...
              {/* Primary CTA Button */}
              <div className="pt-2">
                <Button size="lg" className="group relative overflow-hidden text-base md:text-lg px-8 py-6 h-auto rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-xl hover:brightness-110" style={{
                ...
                border: 'none'
                  ...
                  <span className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-700 group-hover:translate-x-full" style={{
              ...
              <div className="flex flex-wrap items-center gap-6 pt-4">
                <div className="flex items-center gap-2">
                  <div className="flex -space-x-2">
                    {[1, 2, 3, 4].map(i => <div key={i} className="w-8 h-8 rounded-full bg-gradient-to-br from-primary/60 to-primary/40 border-2 border-background ring-2 ring-primary/20" />)}
                  </div>
            ...
            <div className="relative hidden lg:block">
              <div className="relative">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {features.slice(0, 4).map((feature, index) => <Card key={feature.title} className={cn("p-6 border-2 bg-card/50 backdrop-blur-sm transition-all duration-500", "hover:border-primary/50 hover:shadow-xl hover:scale-105", index % 2 === 0 ? "animate-float" : "", index === 0 && "delay-0", index === 1 && "delay-150", index === 2 && "delay-300", index === 3 && "delay-450")} style={{
                ...
                <div className="absolute -bottom-4 -left-4 w-20 h-20 bg-primary/10 rounded-full flex items-center justify-center backdrop-blur-sm border border-primary/20 animate-pulse" style={{
                animationDelay: '1s'
              }}>
      ...
      <section className="py-16 border-y border-border bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
            {metrics.map((metric, index) => <Card key={index} className="text-center group cursor-pointer p-6 rounded-xl border-2 border-border bg-card transition-all duration-300 hover:scale-105 hover:shadow-lg hover:-translate-y-1 hover:border-primary/40">
                <metric.icon className="w-8 h-8 md:w-10 md:h-10 mx-auto mb-3 transition-all duration-300 group-hover:scale-110 group-hover:rotate-6" style={{
              color: '#3B82F6'
            }} />
      ...
      <section className="py-20 md:py-32 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {features.map((feature, index) => <Link key={feature.title} to={feature.to} onMouseEnter={() => setHoveredFeature(index)} onMouseLeave={() => setHoveredFeature(null)}>
                <Card className={cn("relative overflow-hidden p-8 border-2 transition-all duration-500 group cursor-pointer h-full", "hover:shadow-2xl hover:scale-[1.02]", hoveredFeature === index ? `border-primary/50 bg-gradient-to-br ${feature.gradient}` : "border-border bg-card")}>
                  {/* Animated Background Gradient */}
                  <div className={cn("absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-br", feature.gradient)} />

                  <div className="relative z-10">
                    <div className="flex items-start justify-between mb-6">
                      <div className={cn("p-4 rounded-2xl border-2 transition-all duration-300", hoveredFeature === index ? "border-primary/50 bg-primary/10 scale-110 rotate-6" : "border-border bg-muted/50 group-hover:scale-105")}>
                        <feature.icon className="w-8 h-8 text-primary" />
                      </div>
                      <ArrowUpRight className={cn("w-6 h-6 text-muted-foreground transition-all duration-300", "group-hover:text-primary group-hover:translate-x-1 group-hover:-translate-y-1")} />
                    </div>
      ...
      <section className="py-20 md:py-32 bg-gradient-to-b from-transparent via-muted/20 to-transparent">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {capabilities.map((capability, index) => <Card key={index} className="p-6 text-center border border-border bg-card/50 backdrop-blur-sm transition-all duration-300 hover:scale-110 hover:shadow-lg hover:border-primary/50 group cursor-pointer">
      ...
      <section className="py-20 md:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {testimonials.map((testimonial, index) => <Card key={index} className="relative p-8 border-2 border-border bg-card transition-all duration-300 hover:shadow-2xl hover:scale-105 hover:border-primary/30 group">
                {/* Highlight Badge */}
                ...
                <div className="flex items-center gap-1 mb-6">
                  {[1, 2, 3, 4, 5].map(i => <Star key={i} className="w-4 h-4 fill-yellow-400 text-yellow-400 group-hover:scale-110 transition-transform duration-300" style={{
